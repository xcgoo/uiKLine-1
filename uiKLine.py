...
修改by：八荒六合
20190701 change log：
1. 根据vnpy 2回测结果生成trade_result.csv
2. 自动根据trade_result.csv取K线，并生成交易连线，白色表示盈利，红色表示亏损
   连线支持多笔进场多笔出场，但未考虑锁仓等复杂逻辑。
3. 解决windows10下出错退出的bug
4. 解决鼠标缩放定位不准确的问题
5. 指标数据接口改动，支持用同样的颜色显示通道指标
6. trade_result.csv中的interval列第一行可以指定显示K线的周期，如60m显示60分钟K线，在一分钟上回测策略，显示在60分钟K线上
"""
...
class KLineWidget(KeyWraper):
    ...
    def initUi(self):
        ...
        self.lay_KL.setBorder(color=(100, 100, 100, 250), width=0.8)
    ...
    def init_plot_main(self):
        ...
        self.trans_lines['win']=pg.GraphItem(pen={'color':'w', 'width':2,'style':QtCore.Qt.DotLine})
        self.trans_lines['loss']=pg.GraphItem(pen={'color':'r', 'width':2,'style':QtCore.Qt.DotLine})
        self.pi_main.addItem(self.trans_lines['win'])
        self.pi_main.addItem(self.trans_lines['loss'])
        self.pi_main.setMinimumHeight(350)
    ...
    def init_plot_volume(self):
        ...
        if self.display_vol:
            ...
            self.pi_volume.setMaximumHeight(100)
    ...
    # ----------------------------------------------------------------------
    def add_indicator(self, indicator, is_main=True, same_color=False):
        """
        新增指标信号图
    ...
    # ----------------------------------------------------------------------
    def plot_indicator(self, datas, is_main=True, clear=False, same_color=False):
        """
        刷新指标/信号图( 新数据）
        :param datas: 所有数据
        :param is_main: 是否为主图
        ...
        if is_main:
            for indicator in datas:
                indicator_data = datas[indicator]
                if not isinstance(indicator_data, tuple):
                    indicator_data = indicator_data,
                for i in range(len(indicator_data)):
                    data = indicator_data[i]
                    sub_indicator = indicator+'-'+str(i)
                    if i==len(indicator_data)-1: same_color=False
                    self.add_indicator(sub_indicator, is_main, same_color)            # 逐一添加主图信号/指标
                    self.main_indicator_data[sub_indicator] = data        # 更新组件数据字典
                    # 调用该信号/指标画布(plotDataItem.setData())，更新数据，更新画笔颜色，更新名称
                    self.main_indicator_plots[sub_indicator].setData(
                        data, pen=self.main_indicator_colors[sub_indicator][0], name=sub_indicator)
        else:
            for indicator in datas:
                indicator_data=datas[indicator]
                if not isinstance(indicator,tuple):
                    indicator_data = indicator_data,
                for i in range(len(indicator_data)):
                    data = indicator_data[i]
                    sub_indicator = indicator+'-'+str(i)
                    if i==len(indicator_data)-1: same_color=False
                    self.add_indicator(sub_indicator, is_main, same_color)                      # 逐一增加子图指标/信号
                    self.sub_indicator_data[sub_indicator] = data            # 更新组件数据字典
                    # 调用该信号/指标画布(plotDataItem.setData())，更新数据，更新画笔颜色，更新名称
                    self.sub_indicator_plots[sub_indicator].setData(
                        data, pen=self.sub_indicator_colors[sub_indicator][0], name=sub_indicator)
    ...
    def onUp(self):
        ...
        try:
            ...
            print(u'{}.onUp() exception:{},trace:{}'.format(self.title, str(ex), traceback.format_exc()))
        ...
        if need_plot_arrow:
            ...
            if offset == Offset.OPEN.value:
                if direction == Direction.LONG.value:
                    # buy
                    arrow = pg.ArrowItem(pos=(x, price), angle=180, brush=(255, 0, 0))
                else:
                    # short
                    arrow = pg.ArrowItem(pos=(x, price), angle=180, brush=None ,pen={'color':'g','width':1}, headLen=20, headWidth=20, tailLen=5, tailWidth=2)
            # 平仓信号
            elif offset == Offset.CLOSE.value:
                if direction == Direction.SHORT.value:
                    # sell
                    arrow = pg.ArrowItem(pos=(x, price), angle=0, brush=(255, 0, 0))
                else:
                    # cover
                    arrow = pg.ArrowItem(pos=(x, price), angle=0, brush=None, pen={'color':'g','width':1},tipAngle=30, baseAngle=20, tailLen=5, tailWidth=2)
            if arrow:
    ...
    def add_signals(self, df_trade_list):
        ...
        for idx in df_trade_list.index:
            # 交易时间
            trade_time = df_trade_list['datetime'].loc[idx]
            if not isinstance(trade_time,datetime) and isinstance(trade_time,str):
                trade_time = datetime.strptime(trade_time,'%Y-%m-%d %H:%M:%S')
            price       = df_trade_list['price'].loc[idx]
            direction   = df_trade_list['direction'].loc[idx]
            volume      = df_trade_list['volume'].loc[idx]
            offset      = df_trade_list['offset'].loc[idx]
            # 添加信号
            self.add_signal(t_value=trade_time, direction=direction, offset=offset, price=price, volume=volume)
    ...
    def add_trans(self, tns_dict):
        ...
        pos = [[tns['start_x'],tns['start_price']],[tns['end_x'],tns['end_price']]]

        direction = -1 if tns['tns_type']==Direction.SHORT.value else 1
        if direction*tns['end_price']>direction*tns['start_price']:
            self.list_trans_pos['win']+=pos
            last = len(self.list_trans_pos['win'])-1
            self.list_trans_adj['win'].append([last-1,last])
        else:
            self.list_trans_pos['loss']+=pos
            last = len(self.list_trans_pos['loss'])-1
            self.list_trans_adj['loss'].append([last-1,last])
    ...
    def add_trans_df(self,df_trans):
        ...
        for idx in df_trans.index:
            ...
            # tns_type = Direction.LONG.value if tns_type== 'long' else Direction.SHORT.value
    ...
    def loadData(self, df_datas, main_indicators=[], sub_indicators=[]):
        ...
        self.datas = df_datas[['open', 'close', 'low', 'high', 'volume']].to_records()
        self.axisTime.xdict={}
    ...
    def load_vnpy_data(self, symbol:str, exchange:Exchange, interval:Interval, start:datetime, end:datetime, window:int):
        db_data = database_manager.load_bar_data(symbol, exchange, interval, start ,end)
        xbars = []
        def on_window_bar(x):
            xbars.append(x)
        bg = BarGenerator(on_bar=None, window=window, on_window_bar=on_window_bar, interval=interval)
        for bar in db_data:
            bg.update_bar(bar)
        from dataclasses import asdict
        df = pd.DataFrame([asdict(x) for x in xbars])
        df.rename(columns={'open_price':'open','close_price':'close','low_price':'low','high_price':'high'},inplace=True)
        df.set_index(pd.DatetimeIndex(df['datetime']),inplace=True)
        self.loadData(df)

    def load_all_from_trades(self, trade_path):
        from vnpy.trader.utility import TRADER_DIR
        os.chdir(TRADER_DIR)
        df_trade_list = pd.read_csv(trade_path,encoding='GBK')
        if df_trade_list is None or len(df_trade_list) ==0:
            print(u'dataframe is None or Empty',file=sys.stderr)
            return
        #K线
        interval = df_trade_list['interval'].iloc[0]
        window =  int(re.sub("\D","",interval))
        frame = Interval.MINUTE
        buf = timedelta(minutes=20*window)
        if Interval.HOUR.value in interval:
            frame = Interval.HOUR
            buf = timedelta(hours=20*window)
        elif Interval.DAILY.value in interval:
            frame = Interval.DAILY
            buf = timedelta(days=20*window)
        start = datetime.fromisoformat(df_trade_list['datetime'].iloc[0])-buf
        end = datetime.fromisoformat(df_trade_list['datetime'].iloc[-1])+buf
        self.load_vnpy_data(df_trade_list['symbol'].iloc[0],Exchange(df_trade_list['exchange'].iloc[0]),
                            frame, start, end, window)
        #交易信号以及连线
        open_trades = []
        open_volume = 0

        for trade in tqdm.tqdm(df_trade_list.itertuples(),total=len(df_trade_list), desc='handling trade signals:'):
            ...
            if trade.direction==Direction.LONG.value:
                direction = 1
            open_volume += trade.volume * direction
            open_trades.append(trade)
            if abs(open_volume) < 1e-5:
                for i in range(len(open_trades)):
                    if open_trades[i].offset == Offset.CLOSE.value:
                        cur_close_trade = open_trades[i]
                        for j in range(i):
                            if open_trades[j].offset == Offset.OPEN.value:
                                cur_open_trade = open_trades[j]
                                self.add_trans({
                                    'start_time':datetime.fromisoformat(cur_open_trade.datetime),
                                    'end_time':datetime.fromisoformat(cur_close_trade.datetime),
                                    'start_price':cur_open_trade.price,
                                    'end_price':cur_close_trade.price,
                                    'tns_type':cur_open_trade.direction,
                                    'completed':True})
                open_trades = []
                open_volume = 0
        self.trans_lines['win'].setData(pos=np.array(self.list_trans_pos['win']),
                                        adj=np.array(self.list_trans_adj['win']))
        self.trans_lines['loss'].setData(pos=np.array(self.list_trans_pos['loss']),
                                        adj=np.array(self.list_trans_adj['loss']))
...
if __name__ == '__main__':
    ...
    # K线界面
    try:
        ui = KLineWidget(display_sub=True, display_vol=True)
        ui.showMaximized()
        ui.KLtitle.setText('XBTUSD',size='20pt')

        ui.load_all_from_trades('trade_result.csv')
        Donch20Up = talib.MAX(ui.datas['high'],20)
        Donch20Down=talib.MIN(ui.datas['low'],20)
        ui.plot_indicator({'Donch20':(Donch20Up,Donch20Down)},same_color=True)
        Donch10Up = talib.MAX(ui.datas['high'],10)
        Donch10Down=talib.MIN(ui.datas['low'],10)
        ui.plot_indicator({'Donch10':(Donch10Up,Donch10Down)},same_color=True)
        ATR20 = talib.ATR(ui.datas['high'],ui.datas['low'],ui.datas['close'],timeperiod=20)
        ui.plot_indicator({'ATR':ATR20},False)
        RSI14 = talib.RSI(ui.datas['close'],timeperiod=14)
        ui.plot_indicator({'RSI':RSI14},False)
