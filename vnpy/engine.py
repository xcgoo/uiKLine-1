...
class BacktesterEngine(BaseEngine):
    ...
    def calc_trades_pnl(self, trades, size, rate, slippage, interval):
        ...
        for trade in trades:
            ...
            trade.pnl, trade.net_pnl = 0,0
            trade.commission = trade.price * trade.volume * size * rate
            trade.slippage = trade.volume * size * slippage
            trade.interval = interval
            commission += trade.commission
            slip += trade.slippage

            if trade.offset == Offset.CLOSE:
                avg_cost = turnover/volume
                trade.pnl = trade.volume * (avg_cost - trade.price) * size * direction
                trade.net_pnl = trade.pnl - commission - slip

            volume += trade.volume * direction
            if volume != 0:
                turnover += trade.price * trade.volume * direction
            else:
                turnover = 0
                commission = 0
                slip = 0
    ...
    # 交易写入文件
    #------------------------------------------------
    def trade_to_csv(self, trades : list):
        result_path="trade_result.csv"
        if os.path.exists(result_path):
            os.remove(result_path)
        import csv
        import dataclasses
        with open(result_path, 'w', encoding='GBK', newline='') as f:
            dict_writer=None
            for t in tqdm(trades):
                trade = dataclasses.asdict(t)
                trade['datetime']=t.datetime
                trade['offset'] = t.offset.value
                trade['exchange'] = t.exchange.value
                trade['direction'] = t.direction.value
                trade['pnl'] = t.pnl
                trade['net_npl'] = t.net_pnl
                trade['commission'] = t.commission
                trade['slippage'] = t.slippage
                trade['interval'] = t.interval
                if not dict_writer:
                    dict_writer = csv.DictWriter(f,fieldnames=trade.keys())
